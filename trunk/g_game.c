...
*/
    ...
    if(gamekeydown[key_mission] && !paused)			// IF THE KEY FOR DROP IS PRESSED...
    {								// ...AND THE GAME IS NOT PAUSED:
	    ...
	    gamekeydown[key_mission] = false;					// - RESET THE KEYPRESS
        }
    }

    extern boolean st_showobjective;
    extern boolean st_showinvpop;
    extern boolean st_keystate;
    extern int st_popupdisplaytics;
    extern boolean st_showkeys;
    extern boolean st_displaypopup;
    extern int st_keypage;
    extern boolean st_showkeys;
    extern int runtime;
    extern boolean haskeyinrange;
    extern boolean st_dosizedisplay;

    // THIS IS SOME REALLY CRAZY HACK FOR PSP
    // (CONTROLLING MISSION OBJECTIVES, GUNS INVENTORY & KEY INVENTORY...
    // ...USING ONLY ONE BUTTON BUT KEEP THE WAY HOW IT WORKS)
    if(gamekeydown[key_invpop] && !paused && runtime == 0)	// IF THE [INVENTORY] KEY IS PRESSED...
    {								// ...AND THE GAME IS NOT PAUSED...
	if(!automapactive)					// ...AND THE AUTOMAP IS NOT ACTIVE:
	{
    	    st_keypage = -1;					// - RESET THE KEY PAGES TO DEFAULT
    	    st_popupdisplaytics = false;			// - BUT DON'T SET ANY TICS
    	    st_showkeys = false;				// - AND DON'T SHOW ANY KEYS

            st_showobjective = true;				// - INSTEAD, SHOW THE MISSION OBJ'S 
    	    st_showinvpop = false;				// - BUT DON'T SHOW THE INVENTORY
	    st_displaypopup = true;				// - MAKE THAT POPUP VISIBLE ON SCREEN
	...
	runtime = 1;						// - THEN GO TO THE NEXT STEP
    }
    else if(gamekeydown[key_invpop] && !paused && runtime == 1)	// IF THE [INVENTORY] KEY IS PRESSED...
    {								// ...AND THE GAME IS NOT PAUSED...
	if(!automapactive)					// ...AND THE AUTOMAP IS NOT ACTIVE:
	{
    	    st_keypage = -1;					// - RESET THE KEY PAGES TO DEFAULT
    	    ...
    	    st_showobjective = false;				// - EVEN DON'T SHOW THE MISSION OBJ'S
    	    st_showinvpop = true;				// - INSTEAD, SHOW THE INVENTORY
	    st_displaypopup = true;				// - MAKE THAT POPUP VISIBLE ON SCREEN
	...
	runtime = 2;						// - THEN GO TO THE NEXT STEP
    }
    else if(gamekeydown[key_invpop] && !paused && runtime == 2)	// IF THE [INVENTORY] KEY IS PRESSED...
    {								// ...AND THE GAME IS NOT PAUSED:
        st_showobjective = false;				// - THEN DON'T SHOW THE OBJECTIVES
        st_showinvpop = false;					// - AND EVEN DON'T SHOW THE INVENTORY

        if(!st_keystate)					// IF THE 'KEYS' STATE IS NOT SET:
        {
            st_keystate = true;					// - SET THE 'KEYS' STATE

            if(++st_keypage > 2)				// IF THE KEY PAGES ARE ABOVE STANDARD:
            {
                st_popupdisplaytics = 0;			// - RESET THE TICS THAT SHOW THE POPUP
                st_showkeys = false;				// - DON'T SHOW ANY KEYS
                st_displaypopup = false;			// - DON'T SHOW ANY POPUP
                st_keypage = -1;				// - RESET THE PAGES FOR THE KEYS
            }
        }

        st_popupdisplaytics = 50;				// - SET THE TICS TO SHOW THE POPUP
        st_showkeys = true;					// - SET THE 'SHOW THE KEYS' STATE
        st_displaypopup = true;					// - MAKE THAT POPUP VISIBLE ON SCREEN
	...
	if(st_showkeys || st_popupdisplaytics)			// IF 'SHOW KEYS' STATE / TICS ARE SET:
            ST_drawKeysPopup();					// - DRAW THE KEYS INTO THE POPUP

	if(haskeyinrange)					// IF THERE ARE MORE KEYS TO DISPLAY:
	    runtime = 3;					// - GO TO THE NEXT STEP...
	else							// ...OR ELSE (IF NO MORE KEYS):
	    runtime = 0;					// - GO BACK TO THE FIRST STEP
    }
    else if(gamekeydown[key_invpop] && !paused && runtime == 3)	// IF THE [INVENTORY] KEY IS PRESSED...
    {								// ...AND THE GAME IS NOT PAUSED:
        st_showobjective = false;				// - THEN DON'T SHOW THE OBJECTIVES
        st_showinvpop = false;					// - AND EVEN DON'T SHOW THE INVENTORY
	...
	if(haskeyinrange)					// IF THERE ARE MORE KEYS TO DISPLAY:
	    runtime = 4;					// - GO TO THE NEXT STEP...
	else							// ...OR ELSE (IF NO MORE KEYS):
	    runtime = 0;					// - GO BACK TO THE FIRST STEP
    }
    else if(gamekeydown[key_invpop] && !paused && runtime == 4)	// IF THE [INVENTORY] KEY IS PRESSED...
    {								// ...AND THE GAME IS NOT PAUSED:
        st_showobjective = false;				// - THEN DON'T SHOW THE OBJECTIVES
        st_showinvpop = false;					// - AND EVEN DON'T SHOW THE INVENTORY
	...
	if(st_popupdisplaytics > 1)				// IF THE TICS ARE ABOVE '1':
    	    runtime = 0;					// - GO BACK TO THE FIRST STEP
    }
    if(!gamekeydown[key_invpop])				// IF [INVENTORY] KEY IS NOT PRESSED:
    {
            st_showkeys = false;				// - RESET THE 'SHOW KEYS' STATE
            st_keystate = false;				// - RESET THE DEFINITIVE KEY STATE
    }
    gamekeydown[key_invpop] = false;				// - RESET THE [INVENTORY] KEY STATE
...
//    }
    ...
    if (gamekeydown[key_use] && mousey && !paused && mouselook > 0)	// FOR PSP: mouselook, but...
    ...
    else if (gamekeydown[key_use] && !paused && mouselook > 0)
    {
	cmd->buttons |= BT_USE;
	dclicks = 0;		// clear double clicks if hit use button
    }
